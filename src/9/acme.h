...
// errstr9 is set to the error message. After win_start_events() is called,
// win_poll_event() and/or win_wait_event() must be called repeatedly
// to read the events until win_stop_events() is called.
...
// If events have not been started with win_start_event() or if they have
// subsequently been stopped with win_stop_events without then starting
...
// It is an error to call win_write_event() when events have not yet been
